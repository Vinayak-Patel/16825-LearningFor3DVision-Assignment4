...
class Gaussians:
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ###
            N = scales.shape[0]
            cov_3D = torch.zeros((N, 3, 3), device=scales.device)  # (N, 3, 3)
            scale_squared = scales ** 2
            cov_3D[:, 0, 0] = scale_squared.squeeze()
            cov_3D[:, 1, 1] = scale_squared.squeeze()
            cov_3D[:, 2, 2] = scale_squared.squeeze()
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            N = scales.shape[0]
            S_squared = torch.zeros((N, 3, 3), device=scales.device)
            S_squared[:, 0, 0] = scales[:, 0] ** 2
            S_squared[:, 1, 1] = scales[:, 1] ** 2
            S_squared[:, 2, 2] = scales[:, 2] ** 2
            
            RS = torch.bmm(R, S_squared)
            cov_3D = torch.bmm(RS, R.transpose(1, 2)) # (N, 3, 3)
    ...
    def compute_cov_2D(
        ...
        # of this function?
        view_transform = camera.get_world_to_view_transform()
        W = view_transform.get_matrix()[:, :3, :3]  # (N, 3, 3)
        ...
        # cov_2D = J*(W*cov_3D*W_T)*J_T
        if W.shape[0] == 1:
            W = W.expand(N, -1, -1)
        
        W_cov_3D = torch.bmm(W, cov_3D)  # (N, 3, 3)
        W_cov_3D_W_T = torch.bmm(W_cov_3D, W.transpose(1, 2))  # (N, 3, 3)
        J_W_cov_3D_W_T = torch.bmm(J, W_cov_3D_W_T)  # (N, 2, 3)
        
        cov_2D = torch.bmm(J_W_cov_3D_W_T, J.transpose(1, 2))  # (N, 2, 2)
    ...
    def compute_means_2D(means_3D: torch.Tensor, camera: PerspectiveCameras):
        ...
        # Use the camera's built-in projection function to convert directly to screen space
        projected_points = camera.transform_points_screen(means_3D)
    
        # Extract just the x and y coordinates (removing depth)
        means_2D = projected_points[:, :2]
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        # cov_times_diff = torch.matmul(cov_2D_inverse.unsqueeze(1), diff_reshaped) # (N, H*W, 2, 1)
        # result = torch.matmul(diff_reshaped.transpose(-1, -2), cov_times_diff) # (N, H*W, 1, 1)
        
        # power = -0.5 * result.squeeze(-1).squeeze(-1)  # (N, H*W)
        N = means_2D.shape[0]
        num_points = points_2D.shape[1]
        power = torch.zeros((N, num_points), device=diff.device)
    
        diff = points_2D - means_2D
    
        # Reshape for batch matrix multiplication:
        # diff_expanded: (N, H*W, 2, 1)
        diff_expanded = diff.unsqueeze(-1)
    
        # Expand inverse covariance for broadcasting:
        # cov_2D_inverse: (N, 2, 2) -> (N, 1, 2, 2)
        cov_2D_inverse_expanded = cov_2D_inverse.unsqueeze(1)
    
        # Compute Σ^(-1) * (x - μ) with batch matrix multiplication
        # Result: (N, H*W, 2, 1)
        sigma_inv_times_diff = torch.matmul(cov_2D_inverse_expanded, diff_expanded)
    
        # Compute (x - μ)^T * Σ^(-1) * (x - μ) with batch matrix multiplication
        # diff_expanded_t: (N, H*W, 1, 2)
        # Result: (N, H*W, 1, 1)
        diff_expanded_t = diff_expanded.transpose(-1, -2)
        quadratic = torch.matmul(diff_expanded_t, sigma_inv_times_diff)
    
        # Reshape to (N, H*W) and apply -0.5 factor
        power = -0.5 * quadratic.squeeze(-1).squeeze(-1)
...
class Scene:
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        idxs = valid_idxs[torch.argsort(z_vals[valid_idxs])]  # (N,)
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        opacities_expanded = opacities.unsqueeze(1)
    ...
    def splat(
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation
        weighted_colors = colours * alphas * transmittance  # (N, H, W, 3)
        image = torch.sum(weighted_colors, dim=0)  # (H, W, 3)
        ...
        # weight_sum = torch.sum(alphas * transmittance, dim=0)  # (H, W, 1)
        # depth = torch.where(weight_sum > 1e-5, depth_numerator / (weight_sum + 1e-10), torch.zeros_like(depth_numerator))

...
def colours_from_spherical_harmonics(spherical_harmonics, gaussian_dirs):
    ...
    if shdim > 3:
        ...
        z_expanded = z.unsqueeze(1)  # (N, 1)
        x_expanded = x.unsqueeze(1)  # (N, 1)
        
        colours = colours - SH_C1 * y_expanded * c1 + SH_C1 * z_expanded * c2 - SH_C1 * x_expanded * c3
    ...
    if shdim > 12:
        ...
        xy_expanded = xy.unsqueeze(1)  # (N, 1)
        yz_expanded = yz.unsqueeze(1)  # (N, 1)
        xz_expanded = xz.unsqueeze(1)  # (N, 1)
        
        # Complex expressions
        term1 = (2.0 * zz - xx - yy).unsqueeze(1)  # (N, 1)
        term2 = (xx - yy).unsqueeze(1)            # (N, 1)
        
        colours = colours + SH_C2_0 * xy_expanded * c4 + \
                  SH_C2_1 * yz_expanded * c5 + \
                  SH_C2_2 * term1 * c6 + \
                  SH_C2_3 * xz_expanded * c7 + \
                  SH_C2_4 * term2 * c8
    ...
    if shdim > 27:
        ...
        # Complex expressions (expanded for broadcasting)
        term3 = (3.0 * xx - yy).unsqueeze(1)            # (N, 1)
        term4 = (4.0 * zz - xx - yy).unsqueeze(1)       # (N, 1)
        term5 = (2.0 * zz - 3.0 * xx - 3.0 * yy).unsqueeze(1)  # (N, 1)
        term6 = (xx - 3.0 * yy).unsqueeze(1)           # (N, 1)
        ...
        colours = colours + \
                  SH_C3_0 * y_expanded * term3 * c9 + \
                  SH_C3_1 * xy_expanded * z_expanded * c10 + \
                  SH_C3_2 * y_expanded * term4 * c11 + \
                  SH_C3_3 * z_expanded * term5 * c12 + \
                  SH_C3_4 * x_expanded * term4 * c13 + \
                  SH_C3_5 * z_expanded * term2 * c14 + \
                  SH_C3_6 * x_expanded * term6 * c15

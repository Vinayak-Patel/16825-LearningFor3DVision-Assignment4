...
def optimize_mesh_texture(
    ...
    for i in tqdm(range(total_iter)):
        ...
        ### YOUR CODE HERE ###
        elev = torch.FloatTensor(1).uniform_(*elevation_range)
        azim = torch.FloatTensor(1).uniform_(*azimuth_range)
        dist = torch.FloatTensor(1).uniform_(*dist_range)
        
        R, T = look_at_view_transform(dist=dist.item(), elev=elev.item(), azim=azim.item())
        ...
        rend_rgb = rend[..., :3].permute(0, 3, 1, 2)  # (B, 3, H, W)
        rend_rgb = torch.clamp(rend_rgb, 0.0, 1.0)
        # Encode the rendered image to latents
        latents = sds.encode_imgs(rend_rgb)
        # Compute the loss
        loss = sds.sds_loss(
            latents=latents,
